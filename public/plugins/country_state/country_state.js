...
function set_country(oRegionSel, oCountrySel, oCity_StateSel)
{
  ...
  if (countries[region])
  {
    ...
    countryArr = String(countries[region]).split('|');
...
function set_city_state(oCountrySel, oCity_StateSel)
{
  ...
  if (city_states[country])
  {
    ...
    city_stateArr = String(city_states[country]).split('|');
...
function init(default_country, country_id, state_id) {
	var countries = $.map(city_states, function(element,index) {return index});
	countries = countries.sort();
	var countryList = '';
    for (var i = 0; i < countries.length; i++){
    	if (default_country != null && countries[i] == default_country) {
    		countryList += '<option value="'+ countries[i] +'" selected>'+ countries[i] +'</option>';
    	} else {
    		countryList += '<option value="'+ countries[i] +'">'+ countries[i] +'</option>';
    	}
    }
    $('#' + country_id).html(countryList);

	selectState(country_id, state_id);
}

function selectState(country_id, state_id) {
	country = $('#' + country_id).val();
	var states =  String(city_states[country]).split('|');
	states = states.sort();
	var stateList = '';
    for (var i = 0; i < states.length; i++){
		stateList += '<option value="'+ states[i] +'">'+ states[i] +'</option>';
    }
